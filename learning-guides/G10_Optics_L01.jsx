...
export default function G10OpticsL01() {
    ...
    <div style={{ position: "relative", zIndex: 1, maxWidth: 820, margin: "0 auto", padding: "44px 20px 80px" }}>
      ...
      <Card style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontFamily: F.serif, fontSize: 14, color: C.body, lineHeight: 1.75, marginBottom: 14 }}>
          Geometric optics models light as rays — infinitely thin lines that travel in straight paths until they encounter a surface. This approximation works when λ is much smaller than the objects light interacts with. From the law of reflection and Snell's law of refraction, we derive the behavior of mirrors and lenses: the mirror equation, the thin lens equation, magnification, sign conventions, and ray diagrams.
        </p>
        <p style={{ fontFamily: F.serif, fontSize: 14, color: C.body, lineHeight: 1.75, marginBottom: 14 }}>
          The math backbone is <strong style={{ fontWeight: 500, color: C.accent }}>triangle similarity</strong> — every ray diagram is a geometric construction, and every image distance formula derives from proportional triangles.
        </p>
        <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>{["I · Reflection & Refraction", "II · Mirrors", "III · Lenses"].map((p, i) => <Tag key={i}>{p}</Tag>)}</div>
      </Card>
        ...
        <div className="fade">
          ...
          {part === 0 && (<div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <Card>
              ...
              <p style={{ fontFamily: F.serif, fontSize: 15, color: C.body, lineHeight: 1.75, marginBottom: 14 }}>The refractive index n of a medium is the ratio of the speed of light in vacuum to its speed in that medium:</p>
              <MathBlock>{"n = c / v\n\nc = 3.00 × 10⁸ m/s    (vacuum)\nv = speed in medium      (always < c, so n ≥ 1)"}</MathBlock>
              <div style={{ marginTop: 14 }}>{[["Vacuum", "n = 1 (exact)"], ["Air", "n ≈ 1.0003 ≈ 1"], ["Water", "n ≈ 1.33"], ["Glass (crown)", "n ≈ 1.52"], ["Diamond", "n ≈ 2.42"]].map(([k, v]) => <InfoRow key={k} label={k} value={v} />)}</div>
            </Card>

            <Card>
              <Label style={{ display: "block", marginBottom: 14 }}>Snell's Law — The Derivation</Label>
              <p style={{ fontFamily: F.serif, fontSize: 15, color: C.body, lineHeight: 1.75, marginBottom: 14 }}>
                Snell's law emerges from a deeper principle: <strong style={{ fontWeight: 500, color: C.accent }}>Fermat's principle of least time</strong>. Light takes the path that minimizes travel time — not distance. When crossing from a fast medium to a slow one, light bends to spend less time in the slow medium:
              </p>
              <MathBlock>{"n₁ · sin θ₁ = n₂ · sin θ₂\n\nn₁ sin θ₁ = n₂ sin θ₂    compact form"}</MathBlock>
              <p style={{ fontFamily: F.serif, fontSize: 14, color: C.dim, marginTop: 10, lineHeight: 1.65 }}>If n₂ &gt; n₁ (entering slower medium): sinθ₂ &lt; sinθ₁ → θ₂ &lt; θ₁ → bends toward normal. Reverse for exiting to a faster medium.</p>
            </Card>

            <Card>
              <Label style={{ display: "block", marginBottom: 14 }}>Total Internal Reflection</Label>
              <p style={{ fontFamily: F.serif, fontSize: 15, color: C.body, lineHeight: 1.75, marginBottom: 14 }}>When light in a denser medium (n₁ &gt; n₂) strikes the boundary at an angle exceeding the critical angle, no transmission occurs — all light reflects internally:</p>
              <MathBlock>{"θ_c = arcsin(n₂ / n₁)\n\nCondition: n₁ > n₂  and  θ₁ ≥ θ_c"}</MathBlock>
              <div style={{ marginTop: 14 }}>{[["Water → Air", "θ_c = arcsin(1.00/1.33) ≈ 48.8°"], ["Glass → Air", "θ_c = arcsin(1.00/1.52) ≈ 41.1°"], ["Diamond → Air", "θ_c = arcsin(1.00/2.42) ≈ 24.4°"]].map(([k, v]) => <InfoRow key={k} label={k} value={v} />)}</div>
          ...
          {part === 1 && (<div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <Card>
              ...
              <MathBlock>{"1/f = 1/d_o + 1/d_i\n\nf = R/2          focal length = half radius of curvature\n\nm = −d_i/d_o = h_i/h_o     magnification"}</MathBlock>
              <div style={{ marginTop: 14 }}>
                <Label style={{ display: "block", marginBottom: 10 }}>Sign Convention (Cartesian)</Label>
                {[["f &gt; 0", "Concave mirror (converging)"], ["f &lt; 0", "Convex mirror (diverging)"], ["d_o &gt; 0", "Object in front of mirror (always)"], ["d_i &gt; 0", "Real image (in front of mirror)"], ["d_i &lt; 0", "Virtual image (behind mirror)"], ["m &gt; 0", "Upright image"], ["m &lt; 0", "Inverted image"]].map(([k, v]) => <InfoRow key={k} label={k} value={v} />)}
              </div>
            </Card>

            <Card>
              <Label style={{ display: "block", marginBottom: 14 }}>Concave Mirror — Image Cases</Label>
          ...
          {part === 2 && (<div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <Card>
              ...
              <MathBlock>{"1/f = 1/d_o + 1/d_i    thin lens equation\n\nm = −d_i/d_o = h_i/h_o    magnification\n\nP = 1/f                   power in diopters (m⁻¹)"}</MathBlock>
              <div style={{ marginTop: 14 }}>
                <Label style={{ display: "block", marginBottom: 10 }}>Sign Convention for Lenses</Label>
                {[["f &gt; 0", "Converging lens (biconvex)"], ["f &lt; 0", "Diverging lens (biconcave)"], ["d_o &gt; 0", "Object on incoming side (always)"], ["d_i &gt; 0", "Real image (opposite side from object)"], ["d_i &lt; 0", "Virtual image (same side as object)"], ["R &gt; 0", "Center of curvature on outgoing side"], ["R &lt; 0", "Center of curvature on incoming side"]].map(([k, v]) => <InfoRow key={k} label={k} value={v} />)}
              </div>
            </Card>

            <Card>
              <Label style={{ display: "block", marginBottom: 14 }}>Lens Maker's Formula</Label>
              <p style={{ fontFamily: F.serif, fontSize: 15, color: C.body, lineHeight: 1.75, marginBottom: 14 }}>For a thin lens of refractive index n surrounded by air, with surface radii of curvature R₁ (front) and R₂ (back):</p>
              <MathBlock>{"1/f = (n − 1) · (1/R₁ − 1/R₂)\n\nBiconvex:  R₁ > 0, R₂ < 0  →  f > 0\nBiconcave: R₁ < 0, R₂ > 0  →  f < 0"}</MathBlock>
              <p style={{ fontFamily: F.serif, fontSize: 14, color: C.dim, marginTop: 10, lineHeight: 1.65 }}>A lens with equal curvature on both faces (|R₁| = |R₂| = R) has 1/f = 2(n−1)/R for biconvex, f = R/[2(n−1)].</p>
            </Card>

            <Card>
              <Label style={{ display: "block", marginBottom: 14 }}>Image Cases — Converging Lens</Label>
      ...
      {tab === "reference" && (<div className="fade" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <Card>
          <Label style={{ display: "block", marginBottom: 14 }}>Mirrors & Lenses</Label>
          <MathBlock>{"1/f = 1/d_o + 1/d_i           mirror / thin lens equation\nm = −d_i/d_o = h_i/h_o        magnification\nf = R/2                        mirror focal length\nP = 1/f                        lens power (diopters)\n\nLens maker: 1/f = (n−1)(1/R₁ − 1/R₂)"}</MathBlock>
        </Card>
        <Card>
          <Label style={{ display: "block", marginBottom: 14 }}>Sign Convention Summary</Label>
          {[["f &gt; 0", "Concave mirror / Converging lens"], ["f &lt; 0", "Convex mirror / Diverging lens"], ["d_i &gt; 0", "Real image (rays actually converge)"], ["d_i &lt; 0", "Virtual image (rays appear to diverge)"], ["m &gt; 0", "Upright"], ["m &lt; 0", "Inverted"]].map(([k, v]) => <InfoRow key={k} label={k} value={v} />)}
        </Card>
      </div>)}

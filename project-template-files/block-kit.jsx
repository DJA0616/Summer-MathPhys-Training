...
// ═══════════════════════════════════════════════════════════════
// MATH MARKUP — auto-render ^ and _ as <sup>/<sub>
// ═══════════════════════════════════════════════════════════════
// Supports: x^2, x^{abc}, x^(3/2), x^-2, q_0, q_{net}, r_(ij).
// Single-char form: digit run with optional ±, or one letter.
// Already-Unicode subs/sups (q₀, r², ε₀, 10⁻¹²) pass through untouched.

const SUP_STYLE = { fontSize: "0.72em", verticalAlign: "0.45em", lineHeight: 0 };
const SUB_STYLE = { fontSize: "0.72em", verticalAlign: "-0.25em", lineHeight: 0 };

function parseMath(s) {
  if (typeof s !== "string" || (s.indexOf("^") < 0 && s.indexOf("_") < 0)) return s;
  const out = [];
  let buf = "";
  let i = 0;
  ...
  while (i < s.length) {
    ...
    if ((ch === "^" || ch === "_") && i + 1 < s.length) {
      ...
      if (nxt === "{") {
        ...
        if (depth === 0) { content = s.slice(i + 2, j); end = j + 1; }
      } else if (nxt === "(") {
        let j = i + 2, depth = 1;
        while (j < s.length && depth > 0) {
          if (s[j] === "(") depth++;
        ...
        if (depth === 0) { content = s.slice(i + 2, j); end = j + 1; }
      } else if (/[0-9]/.test(nxt) || ((nxt === "-" || nxt === "+" || nxt === "−") && i + 2 < s.length && /[0-9]/.test(s[i + 2]))) {
        let j = i + 1;
        if (nxt === "-" || nxt === "+" || nxt === "−") j++;
        while (j < s.length && /[0-9]/.test(s[j])) j++;
        content = s.slice(i + 1, j); end = j;
      } else if (/[a-zA-Z]/.test(nxt)) {
        let j = i + 2;
  ...
  return out.map((p, idx) => {
    ...
    const inner = parseMath(p.c);
    return p.t === "^"
      ? React.createElement("sup", { key: idx, style: SUP_STYLE }, inner)
      : React.createElement("sub", { key: idx, style: SUB_STYLE }, inner);
  });
}

function walkMath(node) {
  if (node == null || node === false || node === true) return node;
  if (typeof node === "string") return parseMath(node);
  ...
  if (Array.isArray(node)) {
    return node.map((c, i) => {
      ...
      if (React.isValidElement(c)) return walkMath(c);
      return c;
    });
  }
  if (React.isValidElement(node)) {
    if (node.props && node.props.children !== undefined && node.type !== "sup" && node.type !== "sub") {
      return React.cloneElement(node, {}, walkMath(node.props.children));
    }
  }
  return node;
}
...
function InfoRow({ label, value, accent = false }) {
  return (
    <div style={{ display: "flex", gap: 16, padding: "9px 0", borderBottom: `1px solid ${C.border}`, alignItems: "baseline" }}>
      <Mono style={{ minWidth: 160, color: C.dim, flexShrink: 0, fontSize: 11 }}>{walkMath(label)}</Mono>
      <span style={{ fontFamily: F.serif, fontSize: 14, color: accent ? C.accent : C.body, lineHeight: 1.5 }}>{walkMath(value)}</span>
    </div>
  );
}
...
function QuoteBlock({ text, author }) {
  return (
    <div style={{ padding: "14px 18px", borderLeft: `2px solid ${C.accent}55`, margin: "4px 0" }}>
      <p style={{ fontFamily: F.serif, fontSize: 15, color: C.dim, fontStyle: "italic", lineHeight: 1.7, marginBottom: 6 }}>"{walkMath(text)}"</p>
...
function HintBox({ children, variant = "info" }) {
  ...
  return (
    <div style={{ padding: "12px 16px", background: bg, border: `1px solid ${borderCol}22`, borderLeft: `2px solid ${borderCol}`, borderRadius: DS.radius }}>
      ...
      <div style={{ fontFamily: F.serif, fontSize: 13, color: C.dim, lineHeight: 1.7 }}>{walkMath(children)}</div>
    </div>
  );
}
...
function SummaryBox({ intro, points }) {
  return (
    <div style={{ padding: "16px 18px", background: C.elevated, border: `1px solid ${C.accent}33`, borderLeft: `2px solid ${C.accent}88`, borderRadius: DS.radius }}>
      ...
      {intro && <p style={{ fontFamily: F.serif, fontSize: 14, color: C.body, lineHeight: 1.65, marginBottom: points ? 12 : 0 }}>{walkMath(intro)}</p>}
      {points && (
        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          {points.map((p, i) => (
            <div key={i} style={{ display: "flex", gap: 10 }}>
              <Mono style={{ color: C.accent, minWidth: 14, fontSize: 11, flexShrink: 0 }}>{i + 1}.</Mono>
              <span style={{ fontFamily: F.serif, fontSize: 13, color: C.dim, lineHeight: 1.55 }}>{walkMath(p)}</span>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div className="bk-fade" style={{ animationDelay: `${animateIndex * 60}ms`, marginTop: 8, marginBottom: 12 }}>
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: subtitle ? 6 : 0 }}>
        <h2 style={{ fontFamily: F.serif, fontSize: 22, fontWeight: 500, color: C.heading, letterSpacing: "-0.01em" }}>{walkMath(title)}</h2>
        <Tag accent>{title}</Tag>
      </div>
      {subtitle && <p style={{ fontFamily: F.serif, fontSize: 13, color: C.dim, fontStyle: "italic" }}>{walkMath(subtitle)}</p>}
    </div>
  );
}
...
function Equation({ display, inline = false, animateIndex = 0 }) {
  if (inline) {
    return (
      ...
      }}>{walkMath(display)}</span>
    );
  }
  return (
    <MathBlock>
      <span className="bk-fade" style={{ animationDelay: `${animateIndex * 60}ms` }}>{walkMath(display)}</span>
    </MathBlock>
  );
}
    ...
    <div className="bk-fade" style={{ animationDelay: `${animateIndex * 60}ms` }}>
      {!open ? (
        ...
        <div style={{ padding: "10px 14px", background: C.bg, border: `1px solid ${C.border}`, borderLeft: `2px solid ${C.accent}77`, borderRadius: DS.radius }}>
          <p style={{ fontFamily: F.serif, fontSize: 13, color: C.dim, lineHeight: 1.65 }}>{walkMath(children)}</p>
        </div>
      )}
    </div>
  );
...
function Example({ label = "Example", children, animateIndex = 0 }) {
  return (
    <div className="bk-fade" style={{
      ...
      <div style={{ fontFamily: F.serif, fontSize: 14, color: C.body, lineHeight: 1.7 }}>{walkMath(children)}</div>
    </div>
  );
}
...
function MCField({ question, choices, correct, pts = 3, diff = 1, explain, animateIndex = 0, onResult, suppressExplain }) {
  ...
  return (
    <div className="bk-fade" style={{ animationDelay: `${animateIndex * 60}ms` }}>
      ...
      <p style={{ fontFamily: F.serif, fontSize: 14, color: C.body, lineHeight: 1.65, marginBottom: 14 }}>{walkMath(question)}</p>
        ...
        {choices.map((ch, i) => {
          ...
          return (
            ...
            >
              ...
              <span style={{ fontFamily: F.serif, fontSize: 14, color: ok ? C.accentAlt : bad ? C.err : on ? C.accent : C.body, flex: 1, lineHeight: 1.5 }}>{walkMath(ch)}</span>
      ...
      {showExplain && explain && (
        <div style={{ marginTop: 10, padding: "10px 14px", background: C.bg, border: `1px solid ${C.border}`, borderLeft: `2px solid ${C.accent}88`, borderRadius: DS.radius }}>
          <p style={{ fontFamily: F.serif, fontSize: 13, color: C.dim, lineHeight: 1.65 }}>{walkMath(explain)}</p>
        </div>
      )}
    </div>
  );
...
function INTField({ question, answer, pts = 4, diff = 2, hint, animateIndex = 0, onResult, suppressHint }) {
  ...
  return (
    <div className="bk-fade" style={{ animationDelay: `${animateIndex * 60}ms` }}>
      ...
      <p style={{ fontFamily: F.serif, fontSize: 14, color: C.body, lineHeight: 1.65, marginBottom: 16 }}>{walkMath(question)}</p>
      ...
      {showHint && hint && (
        <div style={{ marginTop: 10, padding: "10px 14px", background: C.bg, border: `1px solid ${C.border}`, borderLeft: `2px solid ${C.accent}88`, borderRadius: DS.radius }}>
          <p style={{ fontFamily: F.serif, fontSize: 13, color: C.dim, lineHeight: 1.65 }}>{walkMath(hint)}</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="bk-fade" style={{ animationDelay: `${animateIndex * 60}ms` }}>
      ...
      <div style={{
        ...
        {equation.map((part, i) => {
          ...
          return <span key={i}>{walkMath(part)}</span>;
        })}
      </div>
      ...
      {checked && isCorrect && correctDisplay && <Mono style={{ display: "block", marginTop: 10, color: C.accentAlt, fontSize: 10, letterSpacing: "0.1em" }}>✓ {walkMath(correctDisplay)}</Mono>}
...
export {
  ...
  HighscoresPage,
  SubTabs, ReferenceList, SyllabusPart, SyllabusComposer,
};
